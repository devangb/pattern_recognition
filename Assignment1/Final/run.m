% pair or all
% all = 1 and pair = 0
% bayes('bayes/ls/all/');
% bayes('bayes/ls/pair/12/');
% bayes('bayes/ls/pair/13/');
% bayes('bayes/ls/pair/23/');
% 
% bayes('bayes/nls/interlock/all/');
% bayes('bayes/nls/ring/all/');
% bayes('bayes/nls/spiral/all/');
% 
% bayes('bayes/over/all/');
% bayes('bayes/over/pair/12/');
% bayes('bayes/over/pair/13/');
% bayes('bayes/over/pair/14/');
% bayes('bayes/over/pair/23/');
% bayes('bayes/over/pair/24/');
% bayes('bayes/over/pair/34/');

% bayes('bayes/real/all/');
% bayes('bayes/real/pair/12/');
% bayes('bayes/real/pair/13/');
% bayes('bayes/real/pair/23/');

naivebayes('naivebayes/ls/all/');
naivebayes('naivebayes/ls/pair/12/');
naivebayes('naivebayes/ls/pair/13/');
naivebayes('naivebayes/ls/pair/23/');

naivebayes('naivebayes/nls/interlock/all/');
naivebayes('naivebayes/nls/ring/all/');
naivebayes('naivebayes/nls/spiral/all/');

% naivebayes('naivebayes/over/all/');
% naivebayes('naivebayes/over/pair/12/');
% naivebayes('naivebayes/over/pair/13/');
% naivebayes('naivebayes/over/pair/14/');
% naivebayes('naivebayes/over/pair/23/');
% naivebayes('naivebayes/over/pair/24/');
% naivebayes('naivebayes/over/pair/34/');
% 
% naivebayes('naivebayes/real/all/');
% naivebayes('naivebayes/real/pair/12/');
% naivebayes('naivebayes/real/pair/13/');
% naivebayes('naivebayes/real/pair/23/');
